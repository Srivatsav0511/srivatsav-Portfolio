  ...
  return (
    <article className="min-h-screen bg-white text-zinc-900 selection:bg-zinc-200">
      ...
      <header className="max-w-4xl mx-auto px-6 pt-28 pb-10 text-center md:text-left">
        ...
        <h1 className="mt-4 text-5xl md:text-7xl font-bold tracking-tight text-black">MoneyFormula</h1>
        <p className="mt-4 text-xl text-zinc-600">Built for fun, made to help people calculate money decisions in seconds.</p>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 pb-20 space-y-12">
        ...
        <section className="space-y-4">
          ...
          <p className="text-zinc-700 leading-relaxed">
            AI support was used mainly for drafting UI copy and accelerating repetitive code tasks, while formula logic, validation behavior, and result presentation
            were manually reviewed to maintain correctness.
          </p>
        </section>

        <div className="flex flex-wrap items-center gap-3">
          <a href="https://apps.apple.com/us/search?term=MoneyFormula" target="_blank" className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-black text-white font-bold hover:bg-zinc-800 transition-colors">
            View Product
            <ArrowUpRight size={16} />
          </a>

          <Link href="/privacy/moneyformula" className="inline-flex items-center gap-2 px-6 py-3 rounded-full border border-zinc-300 text-zinc-800 font-bold hover:bg-zinc-100 transition-colors">
            Privacy Policy
          </Link>
        </div>
      </main>
